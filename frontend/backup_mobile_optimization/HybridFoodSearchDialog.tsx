...
import QueryTranslationInfo from './QueryTranslationInfo';
    ...
    if (newQuery.length >= 2) {
      debounceTimeout.current = setTimeout(() => {
        ...
        const searchFn = async () => {
          try {
            // Traduci la query se necessario
            const preparedQuery = hybridFoodService.prepareSearchQuery(newQuery);
            setTranslatedQuery(preparedQuery);
            setShowTranslation(preparedQuery !== newQuery && preparedQuery.trim() !== '');
            
            let results;
            if (useCombinedSearch) {
              results = await hybridFoodService.searchCombinedFoods(newQuery);
            } else {
              results = await hybridFoodService.searchFoods(newQuery);
            }
                      ...
                      <Box component="li" {...props} sx={{ py: isMobile ? 1.5 : 1 }}>
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
                            ...
                            {option.source && (
                              <Chip 
                                label={hybridFoodService.getSourceName(option.source)} 
                                size="small" 
                                variant="outlined"
                              />
                            )}
